    ...
    for type in '' '-Frappe' '-Macchiato'; do
        case "$theme" in
            ...
            -Flamingo)
                ...
                ;;
            -Grey)
                theme_color_dark='#a6adc8'  # Macchiato Grey
                theme_color_light='#585b70' # Latte Grey
                ;;
            -Green)
        ...
        if [[ "$type" == '-Frappe' ]]; then
            background_light='#eff1f5' # Frappe background light
            background_dark='#303446'  # Frappe background dark
            base_dark='#11111b'        # Frappe base dark
            surface_dark='#414559'     # Frappe surface dark
        ...
        if [[ "$type" == '-Macchiato' ]]; then
            background_light='#eff1f5' # Macchiato background light
            background_dark='#24273a'  # Macchiato background dark
            base_dark='#11111b'        # Macchiato base dark
            surface_dark='#363a4f'     # Macchiato surface dark
        ...
        if [[ "$type" != '' ]]; then
            ...
            sed -i "s/#eff1f5/${background_light}/g" "assets${theme}${type}.svg"
            sed -i "s/#1e1e2e/${background_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#11111b/${base_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#313244/${surface_dark}/g" "assets${theme}${type}.svg"
        elif [[ "$theme" != '' ]]; then
